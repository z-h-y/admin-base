(function() {
  ...
  Config.$inject = ['$stateProvider', '$urlRouterProvider', '$logProvider', '$provide', '$httpProvider', 'localStorageServiceProvider'];
  Run.$inject = ['$rootScope', '$location', '$http', '$log', 'Restangular', 'Utils', 'Session', 'Config', '$stateProvider', '$urlRouterProvider'];

  function Config($stateProvider, $urlRouterProvider, $logProvider, $provide, $httpProvider, localStorageServiceProvider) {
  ...
  function httpRequestInterceptor($rootScope, Utils, $q) {
    ...
    function addHttpRequest(conf) {
      var reg1 = /admin\/api\/admin\/users\/\S+\/roles/,
        reg2 = /admin\/api\/sessions\/\S+/,
        reg3 = /admin\/api\/admin\/users\/\S+\/permissions/;

      if (!reg1.test(conf.url) && !reg2.test(conf.url) && !reg3.test(conf.url)) {
        currentRequests.push(conf.promiseObj);
      }
    }
