(function() {
  ...
  function RoleCtrl ($rootScope, $scope, DTColumnDefBuilder, Config, Restangular, Role, Permission, Utils, Codec) {
    ...
    function _getPermissionCategory(permissionName) {
      ...
      if (index !== -1) {
        return permissionName.substring(0, index);
      }
    }

    function _getCategoryItem(permissionName) {
      var index = permissionName.indexOf(':');
      if (index !==-1) {
        var permissionName = permissionName.substring(index+1);
      }
      var tIndex = permissionName.indexOf(':');
      return permissionName.substring(0, tIndex);
    }
    ...
    function _getPermissionCategories(permissionList) {
      ...
      _.each(permissionList, function(permission) {
        ...
        if (name && !_.includes(categorieNames, name)) {
          categorieNames.push(name);
        }
        if (!categories[name] || categories[name].length === 0) {
          categories[name] = [];
        }
        var itemName = _getCategoryItem(permission.displayName);
        if (itemName && !_.includes(categories[name], itemName)) {
          categories[name].push(itemName);
        }

        $scope.permissionList[index]._categorieName = name;
        $scope.permissionList[index]._itemName = itemName;
        index++;
      });
      _.each(categorieNames, function(name) {
        var obj = {
          categorieNames: name,
    ...
    function changeStatus(index) {
      var _categorieName = $scope.permissionCategories[index].categorieNames;
    ...
    function setRolePermission(index) {
      ...
      Role.one(roleId).getList('permissions').then(function(rolePermissionList) {
        ...
        _.forEach($scope.permissionCategories, function(n, key) {
          ...
          var categorieList_one = _.filter($scope.permissionList, {'_categorieName': n.categorieNames});
          var categorieList_two = _.filter($scope.permissionList, {'_categorieName': n.categorieNames, 'status': status});
